...
def search(keyword, status=None):
    ...
    Args:
        ...
        status: Optional status filter ("open" or "done"). If None, all
                matching tasks are returned regardless of status.

    Returns:
        A list of tasks whose titles contain the keyword, filtered by status
        if provided.
