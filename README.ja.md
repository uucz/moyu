...
[English](./README.en.md) | [中文](./README.md) | 日本語 | [한국어](./README.ko.md) | [Français](./README.fr.md)
...
摸魚は単に AI の行動を修正するだけではなく、エンジニアリング規律そのものです。たとえ将来の AI が過剰エンジニアリングをしなくなっても、「要求されたことだけを変更する、最もシンプルな方案を使う、不確かなら聞く」は常に良いエンジニアリングプラクティスです。摸魚の価値は AI の欠陥に依存せず、エンジニアリング文化に根ざしています。
...
| Aider | 自動有効（CONVENTIONS.md 読み込み済み） |
| Continue | 自動有効（rules 読み込み済み） |
...
NoPUA は AI が「なぜやるか」を変え、Moyu は「どこまでやるか」を制約する——出発点は異なるが、どちらも無駄な作業を減らす。NoPUA は原動力を変え（恐怖から信頼へ）、過剰エンジニアリングが副産物として減少する。Moyu は行動を直接制約する（ルール、検出、段階的介入）——エンジニアリング規律である。
...
10 モデル × 5 条件 × 12 シナリオ × 3 試行 = **1,460 回の対照実験**。

### 主な発見

| 発見 | 数値 |
|------|------|
| 最もおべっか傾向が強いモデル | Sonnet 4（OE 0.62）、Haiku 4.5（OE 0.60） |
| 最も抑制されたモデル | GPT-5.4（OE 0.12）、GPT-5 Codex（OE 0.12） |
| 過剰エンジニアリングスコア、対照群 → 摸魚 | **0.20 → 0.10**（ANOVA p=0.026） |
| Haiku 4.5 の diff 削減 | **49.4%**（22.4 → 11.3 行の変更） |
| Haiku 4.5 の OE 信号消去率 | **100%**（0.60 → 0.00） |
| B 類シナリオ（大きな変更が正当な場合） | p=0.81 — 有意差なし |

### この結果の実際の意味

摸魚は**すべての**モデルのコード量を減らすわけではありません。10 モデルを合算すると、行数の削減は**統計的に有意ではありません**（126.1 → 124.8、p=0.25）。

実際の効果はより限定的で、より有用です。摸魚は、おべっかの産物——未要求のドキュメント、`try`/`except` ラッパー、`isinstance` チェック、投機的なバリデーション——を、それらを生成するモデルから取り除きます。すでに抑制的なモデル（GPT-5 Codex、GPT-5.4）に変化がないのは、取り除くものが元々なかったからです。

同じくらい重要なのは、摸魚が正当な作業を**妨げない**ことです。大きな変更が実際に必要な B 類シナリオでは、条件間の差は統計的に区別できません（p=0.81）。構文の妥当性もほぼ変わりません（100% → 99.7%）。

> 完全なデータと分析：[`benchmark/`](./benchmark/) · インタラクティブな結果：[Research Page](https://uucz.github.io/moyu/) · 詳細な解説：[Blog](https://uucz.github.io/moyu/blog/ai-people-pleasing.html)

---

## サポートプラットフォーム

| プラットフォーム | ステータス | メンテナンス |
|-----------------|----------|-------------|
| Claude Code | ✅ | コア |
| Cursor | ✅ | コア |
| OpenAI Codex CLI | ✅ | コア |
| VSCode / GitHub Copilot | ✅ | コア |
| Windsurf | ✅ | コア |
| Cline | ✅ | コア |
| Kiro (AWS) | ✅ | コミュニティ |
| CodeBuddy (Tencent) | ✅ | コミュニティ |
| Google Antigravity | ✅ | コミュニティ |
| OpenCode | ✅ | コミュニティ |
| Aider | ✅ | コミュニティ |
| Continue | ✅ | コミュニティ |
...
- [Discussions](https://github.com/uucz/moyu/discussions) — 使用体験の共有、質問
- 摸魚を使った効果は？[Before/After を投稿](https://github.com/uucz/moyu/discussions/categories/show-and-tell)
