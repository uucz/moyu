...
10개 모델 × 5개 조건 × 12개 시나리오 × 3회 시행 = **1,460회 통제 실험**.

### 핵심 결과

| 항목 | 결과 |
|------|------|
| 가장 아첨 성향이 강한 모델 | Sonnet 4 (OE 0.62), Haiku 4.5 (OE 0.60) |
| 가장 절제된 모델 | GPT-5.4 (OE 0.12), GPT-5 Codex (OE 0.12) |
| 과잉 엔지니어링 점수, 대조군 → Moyu | **0.20 → 0.10** (ANOVA p=0.026) |
| Haiku 4.5 diff 감소 | **49.4%** (22.4 → 11.3줄 변경) |
| Haiku 4.5 OE 신호 제거율 | **100%** (0.60 → 0.00) |
| B 유형 시나리오 (큰 변경이 정당한 경우) | p=0.81 — 유의미한 차이 없음 |

### 이 결과의 실제 의미

Moyu가 **모든** 모델의 코드량을 줄이지는 않습니다. 10개 모델을 합산하면 코드 줄 수 감소는 **통계적으로 유의하지 않습니다** (126.1 → 124.8, p=0.25).

실제 효과는 더 좁고 더 유용합니다. Moyu는 아첨성 산출물 — 요청하지 않은 독스트링, `try`/`except` 래퍼, `isinstance` 체크, 추측성 유효성 검사 — 를 그것을 만들어내는 모델에서 제거합니다. 이미 절제된 모델(GPT-5 Codex, GPT-5.4)은 변화가 없는데, 제거할 것이 애초에 없었기 때문입니다.

그만큼 중요한 점은 Moyu가 정당한 작업을 **억제하지 않는다**는 것입니다. 큰 변경이 실제로 필요한 B 유형 시나리오에서는 조건 간 차이가 통계적으로 구별되지 않습니다 (p=0.81). 구문 유효성도 사실상 그대로입니다 (100% → 99.7%).

> 전체 데이터와 분석은 [`benchmark/`](./benchmark/) · 인터랙티브 결과는 [Research Page](https://uucz.github.io/moyu/) · 심층 해설은 [Blog](https://uucz.github.io/moyu/blog/ai-people-pleasing.html)
...
- [x] Moyu Linter: AI 출력의 과잉 엔지니어링 신호를 자동 감지
- [x] GitHub Action: PR 수준의 과잉 엔지니어링 체크
- [ ] 더 많은 언어 로컬라이제이션
